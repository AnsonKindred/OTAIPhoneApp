          ...
          viewController:(UIViewController*)viewController
{
    ...
    {
        [self doneAuthenticating:nil auth:OTAYouTube.youTubeService.authorizer error:nil];
    }
}
                      ...
                      error:(NSError*)error
{
    ...
    if(globals.username == nil || globals.username == @"")
    {
        ...
        [OTAYouTube.youTubeServiceGData fetchEntryWithURL:[NSURL URLWithString:@"https://gdata.youtube.com/feeds/api/users/default"]
                                        completionHandler:^(GDataServiceTicket *ticket, GDataEntryBase *userProfileEntry, NSError *error)
        {
            ...
            query.mine = YES;
            query.maxResults = 1;
            [OTAYouTube.youTubeService executeQuery:query
                          ...
                          error:(NSError*)error
{
    ...
    [invocation setArgument:OTAYouTube.youTubeService.authorizer atIndex:3];
    if(error != nil)
    {
...
{
    return ((GTMOAuth2Authentication*)OTAYouTube.youTubeService.authorizer).canAuthorize;
}
